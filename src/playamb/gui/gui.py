...
class AudioPlayerGUI:
    ...
    def setup_style(self):
        """Configure ttk styles and basic root window appearance.

        Sets theme, colors and font styles for the application's widgets so
        that subsequent widget creation can rely on named styles.
        """
    ...
    def create_widgets(self):
        """Create and layout the main GUI widget structure.

        Builds the scrollable canvas, top-level labels and the principal
        configuration cards by delegating to specialized widget-creator
        methods such as :meth:`create_input_card` and
        :meth:`create_decoder_settings_card`.
        """
    ...
    def on_scroll_frame_configure(self, _event=None):
        ...
        _event : tkinter.Event, optional
            Event object passed by the ``<Configure>`` binding (unused).
        """
        self.scroll_canvas.configure(scrollregion=self.scroll_canvas.bbox("all"))

    def on_scroll_canvas_configure(self, event):
        """Resize the inner window to match the canvas width.

        Parameters
        ----------
        event : tkinter.Event
            Event generated by the canvas ``<Configure>`` binding containing
            the new widget width.
        """
        self.scroll_canvas.itemconfigure(self.scroll_window, width=event.width)

    def _is_inside_scroll_frame(self, event):
        """Return True if the mouse cursor is over the scrollable frame.

        This helper converts the event coordinates to a widget and walks the
        widget parent chain to decide whether the cursor is contained inside
        the scrollable frame. It is used to prevent the global mouse-wheel
        bindings from scrolling when the cursor is over other popups.

        Parameters
        ----------
        event : tkinter.Event
            The mouse event received by the global binding.

        Returns
        -------
        bool
            ``True`` when the cursor is located within ``self.scroll_frame``.
        """
        try:
    ...
    def on_mousewheel(self, event):
        """Handle global mouse-wheel events to scroll the main canvas.

        The handler only scrolls when the cursor is located over the
        scrollable frame. It also normalizes differences between platforms
        (X11 vs Windows/macOS) and closes open dropdowns to avoid visual
        inconsistencies when scrolling the page.

        Parameters
        ----------
        event : tkinter.Event
            The mouse wheel event forwarded by the ``bind_all`` registration.
        """
    ...
    def create_input_card(self, parent):
        """Create the input selection card with Load AmbiX control.

        Parameters
        ----------
        parent : tkinter.Widget
            Parent container where the card will be placed.
        """
    ...
    def create_playback_card(self, parent):
        """Create the playback control card (play/pause/stop, progress).
    ...
    def create_decoder_settings_card(self, parent):
        """Create the decoder settings card containing HRTF and buffer controls.
    ...
    def create_rotation_card(self, parent):
        """Create the rotation and head-tracking configuration card.
    ...
    def __no_scroll(self, event):
        """Prevent default mouse-wheel behavior for combobox widgets.

        This handler is bound to combobox instances to stop accidental value
        changes when the user scrolls the page.

        Parameters
        ----------
        event : tkinter.Event
            The mouse wheel event; the return value ``'break'`` tells Tkinter
            not to perform the default action.
        """
    ...
    def get_tracking_modes(self):
        """Return the list of available tracking modes for the current system.

        The returned list always contains ``'Off'`` and ``'Demo'``. ``'Hardware'``
        and ``'OSC'`` are included when the corresponding trackers report
        availability.

        Returns
        -------
        list of str
            Tracking mode names suitable for populating the UI combobox.
        """
        modes = ["Off"]
    ...
    def refresh_hardware_tracking_status(self, show_message=True):
        """Refresh availability state of hardware and OSC head trackers.

        This updates internal flags and the small status text displayed in the
        GUI. When ``show_message`` is ``True`` the info panel is refreshed so
        the user can see the current availability immediately.

        Parameters
        ----------
        show_message : bool, optional
            When ``True`` update the info text in the GUI. Default is
            ``True``.
        """
        self.midi_error = ""
    ...
    def create_rotation_slider(self, parent, label, variable, row):
        """Create a labeled rotation slider (yaw/pitch/roll).

        Parameters
        ----------
        parent : tkinter.Widget
            Parent container where the slider will be placed.
        label : str
            Text label for the slider.
        variable : tkinter.Variable
            Variable bound to the slider position.
        row : int
            Grid row where the slider and label will be placed.
    ...
    def create_info_card(self, parent):
        """Create the information card that displays signal and GUI status.
    ...
    def start_stop_callback(self):
        """Callback toggling head tracking when the start/stop button is pressed.

        If tracking is currently stopped this starts it, otherwise it stops
        tracking and optionally resets orientation.
        """
    ...
    def zero_tracker_callback(self):
        """Callback invoked to zero the tracker coordinate frame.

        For OSC-based trackers the zero command is sent via OSC; for hardware
        trackers the tracker is re-started which triggers a zero operation.
        """
    ...
    def tracking_mode_change_callback(self, *args):
        """Callback called when the tracking mode selection changes.

        Currently the implementation stops any active tracking session and
        leaves the GUI in a safe state; the new mode is applied when the user
    ...
    def has_player(self):
        """Return whether a player object has been created.

        Returns
        -------
        bool
            ``True`` when ``self.player`` is not ``None``.
        """
        return self.player is not None

    def has_loaded_player(self):
        """Return whether a player is present and has finished loading.

        Returns
        -------
        bool
            ``True`` when a player exists and ``player.is_loaded`` is ``True``.
        """
    ...
    def set_controls_enabled(self, enabled: bool):
        ...
        ----------
        enabled : bool
            When ``True`` widgets are set to ``tk.NORMAL``, otherwise they
            are disabled.
        """
    ...
    def set_decoder_settings_enabled(self, enabled: bool):
        """Enable or disable the decoder settings widgets.

        Parameters
        ----------
        enabled : bool
            When ``True`` comboboxes and buttons become editable; otherwise
            they are disabled to prevent changes while playback is active.
        """
    ...
    def _normalize_hrtf_path(self, path):
        """Resolve and normalise an HRTF path chosen in the GUI.

        Accepts a bare filename, a relative path (which is resolved relative
        to the application directory) or the special GUI value
        ``'Default FABIAN HRTF'`` which maps to ``None`` here.

        Parameters
        ----------
        path : str or None
            Path as selected in the GUI combobox or button.

        Returns
        -------
        str or None
            Absolute filesystem path to the HRTF file, or ``None`` when the
    ...
    def _update_apply_button(self):
        """Enable or disable the "Apply Settings" button based on playback.

        The button is only enabled when a player is loaded and the audio
        pipeline is not actively playing or paused (i.e. fully stopped).
        """
    ...
    def apply_decoder_settings(self):
        """Apply the currently selected decoder (HRTF / order / buffer) settings.

        This method stops playback, shows a progress note to the user and
        dispatches a background worker to rebuild or reuse cached decoder
        objects. Early returns when no player is loaded.
        """
    ...
    def set_loading(self, loading: bool, message=""):
        """Set the GUI into a loading or normal state.

        When ``loading`` is ``True`` many interactive controls are disabled to
        prevent the user from modifying settings while a background task is
        running. The GUI labels are updated with an optional status
        ``message``.

        Parameters
        ----------
        loading : bool
            Whether the UI should show a loading state.
        message : str, optional
            Optional status message displayed in the loading text widget.
        """
    ...
    def reset_progress_display(self):
        """Reset the progress slider and time display to the start.

        If no player is loaded the duration is treated as zero.
        """
    ...
    def get_order(self):
        """Return the currently selected Ambisonics order or ``None`` for auto.

        Returns
        -------
        int or None
            An integer order when explicitly selected or ``None`` when
            ``'Auto'`` is chosen.
        """
    ...
    def get_block_size(self):
        """Return the selected audio processing block size (samples).

        Returns
        -------
        int
            Block size in samples as chosen in the GUI.
        """
        return int(self.block_size_value.get())

    def get_selected_hrtf_path(self):
        """Return the HRTF path selected in the GUI.

        The GUI exposes a special value ``'Default FABIAN HRTF'`` which maps to
        a packaged default file (resolved relative to the application
        directory) or a fallback constant from ``playamb.utils.utils``.

        Returns
        -------
        str
            Path to the selected SOFA file or the default identifier.
        """
    ...
    def get_headphone_names(self):
        """Return a list of available headphone filter names.

        The list always contains ``'None'`` and a diffuse-field option. If a
        ``resources/Headphones`` directory is present its subdirectories are
        appended to the list.

        Returns
        -------
        list of str
            Headphone filter names.
        """
    ...
    def select_hrtf_file(self):
        """Open a file dialog to select an HRTF SOFA file and save selection.

        The path selected by the user is set into the ``hrtf_path_value``
        variable so the rest of the GUI and worker threads can access it.
        """
    ...
    def show_wav_disabled_message(self):
        """Show an informational popup explaining why WAV playback is disabled."""
        messagebox.showinfo(
            "Disabled",
    ...
    def update_loaded_settings(self, result):
        """Update the small summary text describing the currently loaded decoder.

        Parameters
        ----------
        result : dict
            Dictionary produced by the loader containing keys such as
            ``'order'``, ``'channels'`` and ``'sample_rate'``.
        """
    ...
    def _handle_error(self, title, error):
        """Generic error handler that logs, stops playback and shows a dialog.

        Parameters
        ----------
        title : str
            Title used for the error dialog.
        error : Exception
            Exception instance to display and log.
        """
        message = str(error)
    ...
    def load_ambix(self):
        """Open a file dialog and load an AmbiX WAV file in a background thread.

        This method gathers the GUI-selected decoder settings, shows a loading
        state and spins up a worker thread that constructs the
        :class:`AmbisonicsFile`, :class:`HRTF` and :class:`SphericalHarmonics`
        objects. The final player is posted back to the main thread via
        ``self._backend_load_queue``.
        """
        if self.is_loading:
            return
    ...
    def update_decoder_settings(self):
        """Apply changes made in the decoder settings UI to the running player.

        The method gathers the requested settings and launches a background
        worker that updates or rebuilds decoder objects as needed. Results
        are reported back to the main thread via ``self._backend_load_queue``.
        """
    ...
    def _process_backend_load_queue(self):
        """Process results from background loader threads.

        This method is scheduled periodically via ``root.after`` and consumes
        messages placed on ``self._backend_load_queue`` by worker threads. It
        updates the GUI and internal player state accordingly.
        """
        try:
    ...
    def play(self):
        """Start or resume playback of the loaded AmbiX player.

        This is a lightweight GUI callback which delegates to the
        :class:`AudioPlayer` instance. It avoids performing long-running
    ...
    def pause(self):
        """Pause playback if a loaded player exists.

        Updates GUI state and the apply-settings button accordingly.
        """
        if not self.has_loaded_player():
            return
        try:
    ...
    def stop(self):
        """Stop playback and reset the playback position to the start.

        If no player is loaded the method returns immediately. GUI widgets and
        the time display are updated to reflect the stopped state.
        """
        if not self.has_loaded_player():
            return
        try:
    ...
    def set_volume(self, value):
        ...
        The GUI slider provides a ``0..1`` value; an exponential curve is
        applied before forwarding the value to the :class:`AudioPlayer`.

        Parameters
        ----------
        value : float or str
            Slider value to convert to a volume in the ``0..1`` range.
        """
    ...
    def set_offset(self):
        """Set the start offset (seek) of the loaded player.

        Validates the user input and seeks the player. Errors produce a
        user-visible dialog via ``_handle_error``.
        """
        if not self.has_loaded_player():
            return
    ...
    def set_loop(self):
        """Enable or disable continuous looping on the current player.

        The loop state is read from the GUI checkbox variable and forwarded
        to :class:`AudioPlayer`.
        """
        if not self.has_loaded_player():
            return
        try:
    ...
    def on_progress_press(self, _event):
        """Mark that the user started dragging the progress slider.

        The flag prevents automatic GUI updates from overriding the user's
        drag position until the release event.
        """
        self.is_dragging_progress = True

    def on_progress_drag(self, value):
        """Update the time display while the user drags the progress slider.
    ...
    def on_progress_release(self, _event):
        """Seek the player to the slider position when the user releases it.

        The method clears the dragging flag after completing the seek.
        """
        if not self.has_loaded_player():
            self.is_dragging_progress = False
            return
    ...
    def update_rotation_label(self):
        """Update the textual rotation summary displayed in the GUI."""
    ...
    def enable_yaw(self):
        """Enable or disable the yaw slider depending on the yaw checkbox.

        If the tracker is running the slider is not modified; when disabling
        the slider its value is reset to zero.
        """
    ...
    def enable_pitch(self):
        """Enable or disable the pitch slider depending on the pitch checkbox.

        Behavior mirrors :meth:`enable_yaw`.
        """
        if not self.rotation_tracker.is_running():
            try:
    ...
    def enable_roll(self):
        """Enable or disable the roll slider depending on the roll checkbox.
    ...
    def apply_rotation(self):
        """Apply the manual rotation values to the shared orientation state.

        The method reads the enabled yaw/pitch/roll sliders, stores the
        resulting orientation in ``OrientationState`` and forwards it to the
        audio rotation backend if a player is loaded.
        """
        # find correct ypr
        yaw = pitch = roll = 0.0
        if self.yaw_check_val.get():
    ...
    def reset_rotation(self):
        """Reset manual rotation sliders to zero and apply the change."""
        self.yaw_value.set(0.0)
        self.pitch_value.set(0.0)
        self.roll_value.set(0.0)
        self.apply_rotation()
    ...
    def apply_orientation_to_audio(self, orientation):
        """Forward an ``Orientation`` to the audio spherical harmonics backend.

        Parameters
        ----------
        orientation : Orientation
            Orientation dataclass instance containing yaw/pitch/roll in
            degrees.
        """
        if self.has_loaded_player():
    ...
    def start_head_tracking(self):
        """Start head tracking according to the current tracking mode.

        This method stops any currently running tracker, selects the target
        tracker implementation (Demo/Hardware/OSC), configures UI elements and
        attempts to start the chosen tracker. Errors are shown to the user via
        a dialog.
        """
        self.refresh_hardware_tracking_status(show_message=False)
    ...
    def stop_head_tracking(self, reset_orientation=True):
        """Stop any active head tracker and restore GUI state.

        Parameters
        ----------
        reset_orientation : bool, optional
            When ``True`` the shared orientation state is reset to zero. The
            default is ``True``.
        """
    ...
    def update_head_tracking_loop(self):
        """Poll the active tracker and push orientation updates into the GUI.

        This method is scheduled with ``root.after`` and runs periodically.
        When a tracker is running it retrieves the latest orientation,
        optionally zeros disabled axes and forwards the orientation to the
        audio backend and GUI widgets.
        """
        if self.rotation_tracker.is_running():
    ...
    def draw_head_tracking_visualizer(self, orientation):
        """Draw the virtual head model rotated by the given orientation.

        Parameters
        ----------
        orientation : Orientation
            Orientation dataclass with yaw/pitch/roll in degrees.
        """
    ...
    def build_info_text(self):
        """Build and return a textual summary of the current signal and GUI state.

        Returns
        -------
        str
            Multi-line text suitable to be displayed in the info panel.
        """
        if not self.has_loaded_player():
            return (
    ...
    def format_head_tracker_devices(self):
        """Return a one-line description of detected head tracker devices."""
    ...
    def write_info_text(self, text):
        """Write the provided text into the read-only info text widget.

        Parameters
        ----------
        text : str
            Text to display in the information panel.
        """
    ...
    def update_info(self):
        """Refresh the information panel with current status text."""
        self.write_info_text(self.build_info_text())

    def update_gui_loop(self):
        """Periodic GUI update loop refreshing time and playback status.

        Scheduled via ``root.after`` this method updates the progress slider
        and playback status at a regular interval.
        """
        if self.has_loaded_player():
    ...
    def format_time(seconds: float):
        """Format seconds as a ``MM:SS.ss`` string.
        ...
        -------
        str
            Formatted minutes and seconds string.
        """
    ...
    def refresh_playback_status(self):
        """Update the human-readable playback status label from player events."""
        if not self.has_loaded_player() or self.is_loading:
            return
    ...
    def on_close(self):
        """Handle application shutdown: stop trackers, close player and exit.

        This method is registered as the WM_DELETE_WINDOW handler and performs
        best-effort cleanup before destroying the root window.
        """
    ...
    def run(self):
        ...
        Registers the close handler and enters Tkinter's event loop. This
        method does not return until the window is closed.
        """
